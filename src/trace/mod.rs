...
pub trait Batcher<K: DdBorrow, V, T, R, Output: Batch<K, V, T, R>> {
...
pub trait Builder<K: DdBorrow, V, T, R, Output: Batch<K, V, T, R>> {
...
pub trait Merger<K: DdBorrow, V, T, R, Output: Batch<K, V, T, R>> {
...
pub mod rc_blanket_impls {
    ...
    impl<K: DdBorrow, V, T, R, B: BatchReader<K,V,T,R>> BatchReader<K,V,T,R> for Rc<B> {
    ...
    pub struct RcBatchCursor<K: DdBorrow, V, T, R, B: BatchReader<K, V, T, R>> {
        phantom: ::std::marker::PhantomData<(K, V, T, R)>,
        cursor: B::Cursor,
    }

    impl<K: DdBorrow, V, T, R, B: BatchReader<K, V, T, R>> RcBatchCursor<K, V, T, R, B> {
    ...
    impl<K: DdBorrow, V, T, R, B: BatchReader<K, V, T, R>> Cursor<K, V, T, R> for RcBatchCursor<K, V, T, R, B> {
        ...
        #[inline] fn seek_key(&mut self, storage: &Self::Storage, key: &K::Borrowed) { self.cursor.seek_key(storage, key) }
    ...
    /// An immutable collection of updates.
    impl<K: DdBorrow,V,T,R,B: Batch<K,V,T,R>> Batch<K, V, T, R> for Rc<B> {
    ...
    pub struct RcBatcher<K: DdBorrow,V,T,R,B:Batch<K,V,T,R>> { batcher: B::Batcher }

    /// Functionality for collecting and batching updates.
    impl<K: DdBorrow,V,T,R,B:Batch<K,V,T,R>> Batcher<K, V, T, R, Rc<B>> for RcBatcher<K,V,T,R,B> {
    ...
    pub struct RcBuilder<K: DdBorrow,V,T,R,B:Batch<K,V,T,R>> { builder: B::Builder }

    /// Functionality for building batches from ordered update sequences.
    impl<K: DdBorrow,V,T,R,B:Batch<K,V,T,R>> Builder<K, V, T, R, Rc<B>> for RcBuilder<K,V,T,R,B> {
    ...
    pub struct RcMerger<K: DdBorrow,V,T,R,B:Batch<K,V,T,R>> { merger: B::Merger }

    /// Represents a merge in progress.
    impl<K: DdBorrow,V,T,R,B:Batch<K,V,T,R>> Merger<K, V, T, R, Rc<B>> for RcMerger<K,V,T,R,B> {
